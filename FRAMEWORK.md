...
|------------|-----------------|------|
| **Regression asymmetry** — `tstat_diff`, `beta_diff`: forward vs reverse predictive OLS at horizon $h$, x $w$ | `build_lead_lag_matrix_gvkey(..., score="tstat_diff" \| "beta_diff")` | Default data-driven $C$ |
| **Cross-correlation / $R^2$ / Granger / Levy** — forward vs reverse pairwise metrics at `max_lag`, x $w$ | `score="cross_corr" \| "regression_r2" \| "granger" \| "levy"`; helpers in `pairwise.py` | Alternative statistical $C$ on the same edges |
